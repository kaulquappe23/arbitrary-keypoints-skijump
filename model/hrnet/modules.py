...
from model.hrnet.blocks import Bottleneck, BasicBlock, BN_MOMENTUM
...
class HighResolutionModule(nn.Module):
    """
    A HighResoltionModule contains several branches and consists of a certain number of residual blocks before it is finished with a fusion layer.
    ...
    def _make_one_branch(self, branch_index, block, num_blocks, num_channels, stride=1):
        """
        Build the branch number branch_index. It contains num_blocks blocks of type block (basic or bottleneck blocks are possible).
        """
        downsample = None
    ...
    def _make_fuse_layers(self):
        ...
        Branch output with lower resolution than the target branch needs to be upsampled, branch output with higher resolution is downsampled with strided convolutions.
...
class Stage(nn.Module):
    """
    Capsules a complete HRNet stage. Each stage has a certain number of HRNet modules. Stage 1 has 1 branch, stage 2 has 2 branches, ...
    """
